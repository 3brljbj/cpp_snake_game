...
UIManager::UIManager(int mapWidth, int mapHeight) {
...
void UIManager::update(int cl, int ml, int gi, int pi, int gu) {
...
bool UIManager::areAllMissionsComplete() const {
    for (const auto& mission : missions) if (!mission.completed) return false;
    return true;
}
void UIManager::checkMissions() {
